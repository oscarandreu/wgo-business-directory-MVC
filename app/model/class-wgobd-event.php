...
class wgobd_Event {
		...
		if( is_numeric( $data ) )
	 	{
			...
			if( ! $post || $post->post_status == 'auto-draft' )
				throw new wgobd_Event_Not_Found( "Post with ID '$data' could not be retrieved from the database." );
			...
			if( $instance != false && is_numeric( $instance ) ) {
				...
				$left_join = 	"LEFT JOIN {$wpdb->prefix}wgobd_event_instances aei ON aei.id = $instance AND e.post_id = aei.post_id ";
			} else {
			...
			$query = $wpdb->prepare(
				"SELECT {$select_sql}" .
				"FROM {$wpdb->prefix}wgobd_events e " .
			...
			if( $event === null || $event->post_id === null )
				throw new wgobd_Event_Not_Found( "Event with ID '$data' could not be retrieved from the database." );
		...
		elseif( is_array( $data ) )
	 	{
			...
			throw new wgobd_Invalid_Argument( "Argument to constructor must be integer, array or null, not '$data'." );
		}
 	}
	...
	public function __get( $name ) {
		global $post, $more, $wgobd_events_helper;
		...
		switch( $name ) {
			...
			case 'short_start_time':
				return $wgobd_events_helper->get_short_time( $this->start );

			case 'short_end_time':
				return $wgobd_events_helper->get_short_time( $this->end );

			case 'short_start_date':
				return $wgobd_events_helper->get_short_date( $this->start );
			...
			case 'short_end_date':
				...
				return $wgobd_events_helper->get_short_date( $this->end - 1 );

			// =========================
			// = Get medium-form dates =
			// =========================
			case 'start_time':
				return $wgobd_events_helper->get_medium_time( $this->start );

			case 'end_time':
				return $wgobd_events_helper->get_medium_time( $this->end );

			// =======================
			// = Get long-form times =
			// =======================
			case 'long_start_time':
				return $wgobd_events_helper->get_long_time( $this->start );

			case 'long_end_time':
				return $wgobd_events_helper->get_long_time( $this->end );

			// =======================
			// = Get long-form dates =
			// =======================
			case 'long_start_date':
				return $wgobd_events_helper->get_long_date( $this->start );

			case 'long_end_date':
				// Subtract 1 second so that all-day events' end date still
				// falls within the logical duration of days (since the end date
				// is always midnight of the following day)
				return $wgobd_events_helper->get_long_date( $this->end - 1 );
			...
			case 'timespan_html':
				...
				if( $this->allday ) {
					...
					$timespan .= '<span class="wgobd-allday-label">';
					$timespan .= __( ' (all-day)', wgobd_PLUGIN_NAME );
					$timespan .= '</span>';
				} else {
					if( $long_end_date != $long_start_date )
			...
			case 'categories_html':
				if( $this->categories_html === null ) {
					...
					foreach( $categories as &$category ) {
						$category =
							'<a class="wgobd-category wgobd-term-id-' . $category->term_id . '" ' .
							( $category->description ? 'title="' . esc_attr( $category->description ) . '" ' : '' ) .
							'href="' . get_term_link( $category ) . '">' .
							$wgobd_events_helper->get_category_color_square( $category->term_id ) . ' ' . esc_html( $category->name ) . '</a>';
					}
			...
			case 'tags_html':
				if( $this->tags_html === null ) {
					...
					foreach( $tags as &$tag ) {
						$tag =
							'<a class="wgobd-tag wgobd-term-id-' . $tag->term_id . '" ' .
			...
			case 'color_style':
			  if( $this->color_style === null ) {
			    ...
			    if( $categories && ! empty( $categories ) )
			      $this->color_style = $wgobd_events_helper->get_event_category_color_style( $categories[0]->term_id, $this->allday );
		    }
			...
			case 'faded_color':
			  if( $this->faded_color === null ) {
			    $categories = wp_get_post_terms( $this->post_id, 'events_categories' );
			    if( $categories && ! empty( $categories ) )
			      $this->faded_color = $wgobd_events_helper->get_event_category_faded_color( $categories[0]->term_id );
		    }
			...
			case 'category_colors':
			  if( $this->category_colors === null ) {
			    $categories = wp_get_post_terms( $this->post_id, 'events_categories' );
			    $this->category_colors = $wgobd_events_helper->get_event_category_colors( $categories );
		    }
			...
			case 'recurrence_html':
				...
				return '<strong>' . esc_html( $wgobd_events_helper->rrule_to_text( $this->recurrence_rules  ) ) . '</strong>';
			case 'exclude_html':
				if( ! $this->exception_dates || empty( $this->exception_dates ) )
					return null;

				return '<strong>' . esc_html( $wgobd_events_helper->exdate_to_text( $this->exception_dates  ) ) . '</strong>';
		}
	}
	 ...
	 * but $update is false, creates a new record in the wgobd_events table of
	 ...
	 * creates a new post AND record in the wgobd_events table for this event.
	 *

...
class wgobd_Agenda_Widget extends WP_Widget
{
	...
	function __construct() {
		parent::__construct(
			'wgobd_agenda_widget',
			__( 'Upcoming Events', wgobd_PLUGIN_NAME ),
			array(
				'description' => __( 'All-in-One Event Calendar: Lists upcoming events in Agenda view', wgobd_PLUGIN_NAME ),
				'class' => 'wgobd-agenda-widget',
			)
		);
	}
	...
	function form( $instance )
	{
		...
		$default = array(
			'title'                  => __( 'Upcoming Events', wgobd_PLUGIN_NAME ),
			'events_per_page'        => 10,
	  ...
	  $get_events        = new WP_Query( array ( 'post_type' => wgobd_POST_TYPE, 'posts_per_page' => -1 ) );
	  $events_options    = $get_events->posts;
		...
		$wgobd_view_helper->display( 'agenda-widget-form.php', $fields );
	}
	...
	function widget( $args, $instance )
	{
		...
		if( $instance['hide_on_calendar_page'] &&
		    is_page( $wgobd_settings->calendar_page_id ) )
			return;

		// Add params to the subscribe_url for filtering by Limits (category, tag)
		$subscribe_filter  = '';
		$subscribe_filter .= $instance['event_cat_ids'] ? '&wgobd_cat_ids=' . join( ',', $instance['event_cat_ids'] ) : '';
		$subscribe_filter .= $instance['event_tag_ids'] ? '&wgobd_tag_ids=' . join( ',', $instance['event_tag_ids'] ) : '';
		$subscribe_filter .= $instance['event_post_ids'] ? '&wgobd_post_ids=' . join( ',', $instance['event_post_ids'] ) : '';
		...
		$args['dates']                  = $dates;
		$args['calendar_url']           = $wgobd_calendar_helper->get_calendar_url( null, $limit );
		$args['subscribe_url']          = wgobd_EXPORT_URL . $subscribe_filter;

		$wgobd_view_helper->display( 'agenda-widget.php', $args );
	}
}

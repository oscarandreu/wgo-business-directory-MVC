...
<span class="wgobd-title-buttons"> <a id="wgobd-today"
	class="wgobd-load-view wgobd-button"
	href="#action=wgobd_oneday&amp;wgobd_post_ids=<?php echo $post_ids ?>"> <?php _e( 'Today', wgobd_PLUGIN_NAME ) ?>
</a> </span>
...
<table class="wgobd-1day-view-original">
	<thead>
		<tr>
			<?php foreach( $cell_array as $date => $day ): ?> 
				<th class="wgobd-weekday <?php if( $day['today'] ) echo 'wgobd-today' ?>">
					<span class="wgobd-weekday-date"><?php echo date_i18n( 'l', $date, true )?> </span>
				</th>
	...
	<tbody>
		<tr class="wgobd-1day">	
			...
			<div class="wgobd-grid-container"><?php for( $hour = 0; $hour < 24; $hour++ ) : ?>
			...
			<div class="wgobd-start-truncator">◤</div>
			<?php endif ?> <?php if( $event->end_truncated ) : ?>
			<div class="wgobd-end-truncator">◢</div>
			<?php endif ?> <?php // Insert post ID for use by JavaScript filtering later ?>
			<input type="hidden" class="wgobd-post-id"
				value="<?php echo $event->post_id ?>" />

			<div class="wgobd-event-popup">
			<div class="wgobd-event-summary"><?php if( $event->category_colors ): ?>
			<div class="wgobd-category-colors"><?php echo $event->category_colors ?></div>
			<?php endif ?> <?php if( $event->post_excerpt ): ?> <strong><?php _e( 'Summary:', wgobd_PLUGIN_NAME ) ?></strong>
			<p><?php echo esc_html( $event->post_excerpt ) ?></p>
			<?php endif ?>
			<div class="wgobd-read-more"><?php esc_html_e( 'click anywhere for details', wgobd_PLUGIN_NAME ) ?></div>
			</div>
			<div class="wgobd-event-popup-bg"><span class="wgobd-event-time"><?php echo esc_html( $event->short_start_time ) ?></span>
			<span class="wgobd-event-title"><?php echo esc_html( apply_filters( 'the_title', $event->post->post_title ) ) ?></span>
			</span></div>
			</div>
			<!-- .event-popup -->
			...
			<div
				class="wgobd-event <?php if( $event->post_id == $active_event ) echo 'wgobd-active-event' ?>">

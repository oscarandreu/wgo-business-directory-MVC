...
/**
 * wgobd_Events_Helper class
 *
 * @package Helpers
 * @author The Seed Studio
 **/
class wgobd_Events_Helper {
	/**
	 ...
	 *
	 * @return wgobd_Event  The associated event object
	 **/
	static function get_event( $post_id )
	{
		$event = wp_cache_get( $post_id, wgobd_POST_TYPE );
		if( $event === false ) {
			...
			$event = new wgobd_Event( $post_id, $instance_id );

			if( ! $event->post_id )
				throw new wgobd_Event_Not_Found( "Event with ID '$post_id' could not be retrieved from the database." );

			// Cache the event data
			wp_cache_add( $post_id, $event, wgobd_POST_TYPE );
		}
		return $event;
	}
	 ...
	 **/
	 function insert_event_in_cache_table( $event ) {
		 ...
		 $wpdb->query(
			 $wpdb->prepare(
				 "INSERT INTO {$wpdb->prefix}wgobd_event_instances " .
				 "       ( post_id,  start,               end                 ) " .
	...
	function get_repeat_patterns() {
		...
		if( !isset( $options ) ) {
			$temp = array(
				' ' => __( 'No repeat', wgobd_PLUGIN_NAME ),
				'1' => __( 'Every day', wgobd_PLUGIN_NAME ),
				'2' => __( 'Every week', wgobd_PLUGIN_NAME ),
				'3' => __( 'Every month', wgobd_PLUGIN_NAME ),
				'4' => __( 'Every year', wgobd_PLUGIN_NAME ),
				'5' => '-----------',
				'6' => __( 'Custom...', wgobd_PLUGIN_NAME ),
			);
			$options = $temp;
		}
		return $options;
	...
	function create_repeat_dropdown( $selected = null ) {
		...
		return $this->create_select_element( 'wgobd_repeat', $options, $selected, array( 5 ) );
	}
	 ...
	 *
	 * @param  wgobd_Event  Event object to parse recurrence rules of
	...
	function create_on_the_select( $f_selected = false, $s_selected = false ) {
		...
		$first_options = array(
			'0' => __( 'first', wgobd_PLUGIN_NAME ),
			'1' => __( 'second', wgobd_PLUGIN_NAME ),
			'2' => __( 'third', wgobd_PLUGIN_NAME ),
			'3' => __( 'fourth', wgobd_PLUGIN_NAME ),
			'4' => '------',
			'5' => __( 'last', wgobd_PLUGIN_NAME )
		);
		$ret = $this->create_select_element( 'wgobd_monthly_each_select', $first_options, $f_selected, array( 4 ) );

		$second_options = array(
			'0'   => __( 'Sunday', wgobd_PLUGIN_NAME ),
			'1'   => __( 'Monday', wgobd_PLUGIN_NAME ),
			'2'   => __( 'Tuesday', wgobd_PLUGIN_NAME ),
			'3'   => __( 'Wednesday', wgobd_PLUGIN_NAME ),
			'4'   => __( 'Thursday', wgobd_PLUGIN_NAME ),
			'5'   => __( 'Friday', wgobd_PLUGIN_NAME ),
			'6'   => __( 'Saturday', wgobd_PLUGIN_NAME ),
			'7'   => '--------',
			'8'   => __( 'day', wgobd_PLUGIN_NAME ),
			'9'   => __( 'weekday', wgobd_PLUGIN_NAME ),
			'10'  => __( 'weekend day', wgobd_PLUGIN_NAME )
		);

		return $ret . $this->create_select_element( 'wgobd_monthly_on_the_select', $second_options, $s_selected, array( 7 ) );
	}
	...
	function create_list_element( $name, $options = array(), $selected = array() ) {
		ob_start();
		?>
		<ul class="wgobd_date_select <?php echo $name?>" id="<?php echo $name?>">
			<?php foreach( $options as $key => $val ): ?>
				<li<?php echo in_array( $key, $selected ) ? 'class="wgobd_selected"' : '' ?>>
					<?php echo $val ?>
	...
	function create_montly_date_select( $selected = array() ) {
		...
		return $this->create_list_element( 'wgobd_montly_date_select', $options, $selected );
	}
	...
	function create_yearly_date_select( $selected = array() ) {
		...
		return $this->create_list_element( 'wgobd_yearly_date_select', $options, $selected );
	}

	function get_frequency( $index ) {
		$frequency = array(
			0 => __( 'Daily', wgobd_PLUGIN_NAME ),
			1 => __( 'Weekly', wgobd_PLUGIN_NAME ),
			2 => __( 'Monthly', wgobd_PLUGIN_NAME ),
			3 => __( 'Yearly', wgobd_PLUGIN_NAME ),
		);
	...
	function row_frequency( $visible = false, $selected = false ) {
		...
		$args = array(
		 'visible'    => $visible,
		 'frequency'  => $this->create_select_element( 'wgobd_frequency', $frequency, $selected )
		);
		return $wgobd_view_helper->get_view( 'row_frequency.php', $args );
	}
	...
	function row_daily( $visible = false, $selected = 1 ) {
		...
		$args = array(
		 'visible'  => $visible,
		 'count'    => $this->create_count_input( 'wgobd_daily_count', $selected, 365 ) . __( 'day(s)', wgobd_PLUGIN_NAME )
		);
		return $wgobd_view_helper->get_view( 'row_daily.php', $args );
	}
	...
	function row_weekly( $visible = false, $count = 1, $selected = array() ) {
		global $wgobd_view_helper, $wp_locale;
		...
		$args = array(
		 'visible'    => $visible,
		 'count'      => $this->create_count_input( 'wgobd_weekly_count', $count, 52 ) . __( 'week(s)', wgobd_PLUGIN_NAME ),
		 'week_days'  => $this->create_list_element( 'wgobd_weekly_date_select', $options, $selected )
		);
		return $wgobd_view_helper->get_view( 'row_weekly.php', $args );
	}
	 ...
	 * @return void
	 **/
	function row_monthly( $visible = false, $count = 1, $wgobd_monthly_each = 0, $wgobd_monthly_on_the = 0, $month = array(), $first = false, $second = false ) {
		global $wgobd_view_helper, $wp_locale;
		$start_of_week = get_option( 'start_of_week', 1 );
		...
		$args = array(
		 'visible'              => $visible,
		 'count'                => $this->create_count_input( 'wgobd_monthly_count', $count, 12 ) . __( 'month(s)', wgobd_PLUGIN_NAME ),
		 'wgobd_monthly_each'   => $wgobd_monthly_each,
		 'wgobd_monthly_on_the' => $wgobd_monthly_on_the,
		 'month'                => $this->create_montly_date_select( $month ),
		 'on_the_select'        => $this->create_on_the_select( $first, $second ),
		 'day_nums'             => $this->create_select_element( 'wgobd_monthly_byday_num', $options_dn ),
		 'week_days'            => $this->create_select_element( 'wgobd_monthly_byday_weekday', $options_wd )
		);
		return $wgobd_view_helper->get_view( 'row_monthly.php', $args );
	}
	...
	function row_yearly( $visible = false, $count = 1, $year = array(), $first = false, $second = false ) {
		...
		$args = array(
		 'visible'              => $visible,
		 'count'                => $this->create_count_input( 'wgobd_yearly_count', $count, 10 ) . __( 'year(s)', wgobd_PLUGIN_NAME ),
		 'year'                 => $this->create_yearly_date_select( $year ),
		 'on_the_select'        => $this->create_on_the_select( $first, $second )
		);
		return $wgobd_view_helper->get_view( 'row_yearly.php', $args );
	}
	...
	function get_all_matching_posts( $s_time, $e_time ) {
		...
		return $wgobd_calendar_helper->get_events_between( $s_time, $e_time );
	}
	...
	function get_matching_events( $start = false, $end = false, $filter = array() ) {
		global $wpdb, $wgobd_calendar_helper;
		...
		$wgobd_calendar_helper->_get_filter_sql( $filter );
		...
		$query = $wpdb->prepare(
			...
			"FROM $wpdb->posts " .
				"INNER JOIN {$wpdb->prefix}wgobd_events AS e ON e.post_id = ID " .
				$filter['filter_join'] .
			"WHERE post_type = '" . wgobd_POST_TYPE . "' " .
		...
		foreach( $events as &$event ) {
			try{
				$event = new wgobd_Event( $event );
			} catch( wgobd_Event_Not_Found $n ) {
				unset( $event );
	 ...
	 *
	 * @param wgobd_Event $event  The event object to display a map for
	 *
	 * @return string
	 **/
	...
	function get_category_color_square( $term_id ) {
		...
		if( ! is_null( $color ) && ! empty( $color ) )
			return '<div class="wgobd-category-color" style="background:' . $color . '" title="' . esc_attr( $cat->name ) . '"></div>';
	...
	function create_end_dropdown( $selected = null ) {
		...
		$options = array(
			0 => __( 'Never', wgobd_PLUGIN_NAME ),
			1 => __( 'After', wgobd_PLUGIN_NAME ),
			2 => __( 'On date', wgobd_PLUGIN_NAME )
		);

		?>
		<select name="wgobd_end" id="wgobd_end">
			<?php foreach( $options as $key => $val ): ?>
				...
				if( $rc->getByDay() ) {
					if( count( $rc->getByDay() ) > 1 ) {
						...
						if( count( $rc->getByDay() ) > 2 ) {
							...
							$txt .= ' ' . _x( 'on', 'Recurrence editor - weekly tab', wgobd_PLUGIN_NAME ) . $_days;
						} else {
							$_days = '';
							foreach( $rc->getByDay() as $d ) {
								$day = $this->get_weekday_by_id( $d, true );
								$_days .= ' ' . $wp_locale->weekday[$day] . ' ' . __( 'and', wgobd_PLUGIN_NAME );
							}
							// remove the last ' and'
			...
			case 'monthly':
				if( $rc->getByMonthDay() ) {
					...
					if( count( $rc->getByMonthDay() ) > 2 ) {
						...
						$_days = substr( $_days, 0, -1 );
						$txt .= ' ' . _x( 'on', 'Recurrence editor - monthly tab', wgobd_PLUGIN_NAME ) . $_days . ' ' . __( 'of the month', wgobd_PLUGIN_NAME );
					} else if( count( $rc->getByMonthDay() ) > 1 ) {
						$_days = '';
						foreach( $rc->getByMonthDay() as $m_day ) {
							$_days .= ' ' . $this->_ordinal( $m_day ) . ' ' . __( 'and', wgobd_PLUGIN_NAME );
						}
						$_days = substr( $_days, 0, -4 );
						$txt .= ' ' . _x( 'on', 'Recurrence editor - monthly tab', wgobd_PLUGIN_NAME ) . $_days . ' ' . __( 'of the month', wgobd_PLUGIN_NAME );
					} else {
					...
					$txt .= ' ' . _x( 'on', 'Recurrence editor - monthly tab', wgobd_PLUGIN_NAME ) . $dnum . $_days;
				}
				break;
			case 'yearly':
				if( $rc->getByMonth() ) {
					...
					if( count( $rc->getByMonth() ) > 2  ) {
						...
						$txt .= ' ' . _x( 'on', 'Recurrence editor - yearly tab', wgobd_PLUGIN_NAME ) . $_months;
					} else if( count( $rc->getByMonth() ) > 1 ) {
						$_months = '';
						foreach( $rc->getByMonth() as $_m ) {
							$_m = $_m < 10 ? 0 . $_m : $_m;
							$_months .= ' ' . $wp_locale->month[$_m] . ' ' . __( 'and', wgobd_PLUGIN_NAME );
						}
		...
		switch( $freq ) {
			case 'daily':
				...
				if( ! $interval || $interval == 1 ) {
					$txt = __( 'Daily', wgobd_PLUGIN_NAME );
				} else {
					if( $interval == 2 ) {
						$txt = __( 'Every other day', wgobd_PLUGIN_NAME );
					} else {
						$txt = sprintf( __( 'Every %d days', wgobd_PLUGIN_NAME ), $interval );
					}
				}
				break;
			case 'weekly':
				// check if interval is set
				if( ! $interval || $interval == 1 ) {
					$txt = __( 'Weekly', wgobd_PLUGIN_NAME );
				} else {
					if( $interval == 2 ) {
						$txt = __( 'Every other week', wgobd_PLUGIN_NAME );
					} else {
						$txt = sprintf( __( 'Every %d weeks', wgobd_PLUGIN_NAME ), $interval );
					}
				}
				break;
			case 'monthly':
				// check if interval is set
				if( ! $interval || $interval == 1 ) {
					$txt = __( 'Monthly', wgobd_PLUGIN_NAME );
				} else {
					if( $interval == 2 ) {
						$txt = __( 'Every other month', wgobd_PLUGIN_NAME );
					} else {
						$txt = sprintf( __( 'Every %d months', wgobd_PLUGIN_NAME ), $interval );
					}
				}
				break;
			case 'yearly':
				// check if interval is set
				if( ! $interval || $interval == 1 ) {
					$txt = __( 'Yearly', wgobd_PLUGIN_NAME );
				} else {
					if( $interval == 2 ) {
						$txt = __( 'Every other year', wgobd_PLUGIN_NAME );
					} else {
						$txt = sprintf( __( 'Every %d years', wgobd_PLUGIN_NAME ), $interval );
					}
				}
				break;
		}
	...
	function _ending_sentence( &$txt, &$rc ) {
		if( $until = $rc->getUntil() ) {
			if( ! is_int( $until ) )
				$until = strtotime( $until );
			$txt .= ' ' . sprintf( __( 'until %s', wgobd_PLUGIN_NAME ), date_i18n( get_option( 'date_format' ), $until, true ) );
		}
		else if( $count = $rc->getCount() )
			$txt .= ' ' . sprintf( __( 'for %d occurrences', wgobd_PLUGIN_NAME ), $count );
		else
			$txt .= ' - ' . __( 'forever', wgobd_PLUGIN_NAME );
	}
	...
	function post_type_link( $permalink, $post, $leavename ) {
		...
		if( $post->post_type == wgobd_POST_TYPE ) {
			$delimiter = $wgobd_app_helper->get_param_delimiter_char( $permalink );
	...
	function get_repeat_box() {
		...
		} catch( wgobd_Event_Not_Found $e ) { /* event wasn't found, keep defaults */ }
		...
		$args = array(
			...
			'count'           => $this->create_count_input( 'wgobd_count', $count ) . __( 'times', wgobd_PLUGIN_NAME ),
		...
		$output = array(
			"error" 	=> false,
			"message"	=> $wgobd_view_helper->get_view( 'box_repeat.php', $args ),
			"repeat"  => $repeat
		);
	...
	function get_date_picker_box() {
		...
		$output = array(
			"error" 	=> false,
			"message"	=> $wgobd_view_helper->get_view( 'box_date_picker.php', $args ),
		);
	...
	function get_week_start_day_offset( $wday ) {
		...
		return - ( 7 - ( $wgobd_settings->week_start_day - $wday ) ) % 7;
	}
}
// END class
...
/**
 * Wgobd_View_Helper class
 *
 * @package Helpers
 * @author The Seed Studio
 **/
class WgoBd_View_Helper {
	/**
	...
	function display_css( $file = false, $args = array() ) {
		...
		if( ! $file || empty( $file ) ) {
			throw new Wgobd_File_Not_Provided( 'You need to specify a css file.' );
		}

		$file = WGOBD_CSS_PATH . "/" . $file;

		if( isset( $displayed[$file] ) && $displayed[$file] === $args )	// Skip if already displayed
			return;

		if( ! file_exists( $file ) ) {
			throw new Wgobd_File_Not_Found( "The specified css file doesn't exist." );
		} else {
	...
	function display_js( $file = false, $args = array() ) {
		...
		if( ! $file || empty( $file ) ) {
			throw new Wgobd_File_Not_Provided( "You need to specify a js file." );
		}

		$file = WGOBD_JS_PATH . "/" . $file;

		if( $displayed[$file] === $args)	// Skip if already displayed
			return;

		if( ! file_exists( $file ) ) {
			throw new Wgobd_File_Not_Found( "The specified js file doesn't exist." );
		} else {
			$displayed[$file] = $args;	// Flag that we've displayed this file with these args
